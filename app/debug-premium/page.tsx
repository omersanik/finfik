    ...
    <div className="container mx-auto p-8 max-w-4xl">
      ...
      <div className="space-y-6">
        <div className="bg-gray-100 p-4 rounded-lg">
          ...
          <p><strong>Email:</strong> {user?.emailAddresses?.[0]?.emailAddress || 'Not loaded'}</p>
          <p><strong>Token:</strong> {token ? 'Exists' : 'Missing'}</p>
        </div>

        <div className="bg-gray-100 p-4 rounded-lg">
          <h2 className="text-xl font-semibold mb-2">React Query Hook</h2>
          <p><strong>Loading:</strong> {premiumLoading ? 'Yes' : 'No'}</p>
          <p><strong>Error:</strong> {premiumError ? premiumError.message : 'None'}</p>
          <p><strong>Data:</strong> {JSON.stringify(premiumData, null, 2)}</p>
        </div>

        <div className="bg-gray-100 p-4 rounded-lg">
          <h2 className="text-xl font-semibold mb-2">Premium Status Testing</h2>
          <p className="mb-4 text-sm text-gray-600">
            <strong>Revoke Premium:</strong> This will remove premium access, delete all premium course enrollments, 
            progress, and redirect you away from premium courses. Use this to test the security.
          </p>
          <p className="mb-4 text-sm text-gray-600">
        ...
        <div className="bg-gray-100 p-4 rounded-lg">
          ...
          <p>Check the browser console for detailed debug logs from the Navbar component.</p>
        </div>
      </div>
    </div>
  );
