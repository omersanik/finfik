...
export async function GET(
  ...
  try {
    ...
    if (userError || !user) {
      console.error(
        `[API] User not found. userId: ${userId}, error:`,
        userError
      );
      return NextResponse.json(
        { error: `User not found for clerk_id: ${userId}` },
        { status: 404 }
      );
    }
    ...
    if (coursePathError || !coursePath) {
      console.error(
        `[API] Course path not found for course ID: ${course.id}, error:`,
        coursePathError
      );
      return NextResponse.json(
        {
          error: `Course path not found for course ID: ${course.id}`,
          details: coursePathError,
        },
        { status: 404 }
      );
    }
    ...
    const hasAnyProgress = sortedSections.some((section) => {
      const progress = (coursePath.course_path_sections || [])
        .find((s) => s.id === section.id)
        ?.course_path_section_progress?.find(
          (p) => p.clerk_id === user.clerk_id
        );
    ...
    if (!hasAnyProgress) {
      console.log(
        `[API] No progress found for user ${user.clerk_id} in course ${course.id}, initializing...`
      );
      ...
      if (initError) {
        ...
        console.log(
          `[API] Progress initialized for user ${user.clerk_id} in course ${course.id}`
        );
    ...
    console.error("[API] Course path route error:", error);
    return NextResponse.json(
