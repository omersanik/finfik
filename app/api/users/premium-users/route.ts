...
export async function GET() {
  ...
  if (allUserData) {
    ...
    console.log('subscription_plan from all data:', allUserData.subscription_plan);
    console.log('subscription_plan type from all data:', typeof allUserData.subscription_plan);
  }
...
export async function PATCH(req: Request) {
  ...
  if (is_premium === false) {
    ...
    try {
      ...
      console.log(`Found ${premiumCourseIds.length} premium courses to clean up`);
      ...
      if (premiumCourseIds.length > 0) {
        ...
        if (enrollmentError) {
          console.error("Error removing course enrollments:", enrollmentError);
        } else {
        ...
        if (!pathError && coursePaths && coursePaths.length > 0) {
          ...
          console.log(`Found ${coursePathIds.length} course paths to clean up`);
          ...
          if (!sectionError && sectionIds && sectionIds.length > 0) {
            ...
            console.log(`Found ${sectionIdList.length} course sections to clean up`);
        ...
        const { data: remainingProgress, error: remainingError } = await supabase
          .from("course_path_section_progress")
          .select("id")
          .eq("clerk_id", userId)
          .limit(1);
        
        if (!remainingError && (!remainingProgress || remainingProgress.length === 0)) {
      ...
      return new Response(JSON.stringify({ 
        success: true, 
        message: "Premium access revoked and all premium course data cleaned up" 
      }), { status: 200 });
      
    } catch (error) {
      console.error("Error during premium cleanup:", error);
      return new Response(JSON.stringify({ error: "Failed to clean up premium access" }), { status: 500 });
    }
  }
  ...
  return new Response(JSON.stringify({ error: "Invalid premium status value" }), { status: 400 });
}
