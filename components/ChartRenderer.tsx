...
export default function ChartRenderer({
  ...
  if (!chartData || chartData.trim() === "") {
    return (
      <div
        className={`p-4 text-center text-gray-500 bg-gray-50 rounded-lg ${className}`}
      >
  ...
  if (!chartData.trim().startsWith("{") || !chartData.includes('"type"')) {
    return (
      <div
        className={`p-4 text-center text-gray-500 bg-gray-50 rounded-lg ${className}`}
      >
        This content is not a chart. Please use the chart editor to create
        charts.
      </div>
    );
  }
  ...
  try {
    ...
    if (!config.type || !config.title || !config.description) {
      return (
        <div
          className={`p-4 text-center text-gray-500 bg-gray-50 rounded-lg ${className}`}
        >
          Chart data is incomplete. Please use the chart editor to create a
          complete chart.
        </div>
      );
    }
    ...
    return (
      <div
        className={`p-4 text-center text-red-500 bg-red-50 rounded-lg ${className}`}
      >
