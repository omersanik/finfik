...
import { Trash2, AlertTriangle, Folder, FileText, Layers } from "lucide-react";
  ...
  const fetchSectionsForCourse = async (courseId: string) => {
    try {
      const sectionsRes = await fetch(
        `/api/admin/sections?courseId=${courseId}`
      );
      if (sectionsRes.ok) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Select courses, sections, blocks, and items to delete content
          </p>
        </div>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleDelete(
                            "contentItem",
                            item.id,
                            item.content_text?.substring(0, 30) ||
                              "Content Item"
                          )
                        }
              ...
              <CardContent className="space-y-3">
                {getSelectedCourse() && (
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      <h3 className="font-medium">
                        Delete Course: {getSelectedCourse()?.title}
                      </h3>
                      <p className="text-sm text-gray-500">
                        This will delete the entire course and all its content
                      </p>
                    </div>
                    <Button
                      variant="destructive"
                    ...
                    >
                      {deleteLoading === selectedCourseId
                        ? "Deleting..."
                        : "Delete Course"}
                    </Button>
                  </div>
                ...
                {getSelectedSection() && (
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      <h3 className="font-medium">
                        Delete Section: {getSelectedSection()?.title}
                      </h3>
                      <p className="text-sm text-gray-500">
                        This will delete the section and all its content blocks
                      </p>
                    </div>
                    <Button
                      variant="destructive"
                    ...
                    >
                      {deleteLoading === selectedSectionId
                        ? "Deleting..."
                        : "Delete Section"}
                    </Button>
                  </div>
                ...
                {getSelectedBlock() && (
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      <h3 className="font-medium">
                        Delete Content Block: {getSelectedBlock()?.title}
                      </h3>
                      <p className="text-sm text-gray-500">
                        This will delete the content block and all its items
                      </p>
                    </div>
                    <Button
                      variant="destructive"
                      onClick={() =>
                        handleDelete(
                          "contentBlock",
                          selectedBlockId,
                          getSelectedBlock()?.title || ""
                        )
                      }
                      disabled={deleteLoading === selectedBlockId}
                    >
                      {deleteLoading === selectedBlockId
                        ? "Deleting..."
                        : "Delete Block"}
                    </Button>
                  </div>
